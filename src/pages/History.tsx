      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-gray-100 shadow-xs flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Review past workouts, sets, volume, and performance trends.
          </p>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {workouts.map((workout) => {
            ...
            return (
              <div
                key={workout.id}
                className="bg-white rounded-3xl border border-gray-100 shadow-xs overflow-hidden transition-all"
              >
                ...
                >
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2 text-xs font-bold text-gray-400">
                      ...
                      {workout.routineName && (
                        <span className="bg-red-50 text-[#FF2625] px-2 py-0.5 rounded-full text-[10px] uppercase font-black">
                          {workout.routineName}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-6 justify-between sm:justify-end">
                    <div className="flex items-center gap-4 text-xs font-bold">
                      <span className="flex items-center gap-1 text-gray-600 bg-gray-100 px-3 py-1.5 rounded-xl">
                        <Clock className="w-3.5 h-3.5 text-gray-400" />
                        {workout.durationMinutes} mins
                      </span>
                      <span className="flex items-center gap-1 text-gray-900 bg-[#FFF3F4] text-[#FF2625] px-3 py-1.5 rounded-xl">
                  ...
                  <div className="px-6 pb-6 pt-2 border-t border-gray-100 bg-gray-50/50 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {Object.entries(exerciseGroups).map(([exName, sets]) => (
                        <div key={exName} className="p-4 rounded-2xl bg-white border border-gray-100 shadow-xs">
