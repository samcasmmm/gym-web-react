...
const Workout: React.FC = () => {
  ...
  const { data: searchResults, isLoading: isSearchLoading } = useExercises(
    selectedCategory,
    modalSearch,
    20
  );
  ...
  if (!isActive) {
    return (
      <div className="max-w-4xl mx-auto py-16 px-4 text-center">
        <div className="w-24 h-24 rounded-3xl bg-red-50 text-[#FF2625] flex items-center justify-center mx-auto mb-6 shadow-inner">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <button
            ...
            className="bg-[#FF2625] hover:bg-[#e0201f] text-white px-8 py-3.5 rounded-xl font-bold text-base shadow-lg shadow-red-500/25 flex items-center gap-2 cursor-pointer transition-all hover:scale-102"
          >
          ...
          <button
            ...
            className="bg-gray-100 hover:bg-gray-200 text-gray-800 px-6 py-3.5 rounded-xl font-bold text-base transition-colors cursor-pointer"
          >
            Pick from Routines
          </button>
        </div>
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4 space-y-8">
      {/* Top Header Bar */}
      <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xs border border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
          {/* Workout Stopwatch */}
          <div className="flex items-center gap-2 bg-gray-900 text-white px-4 py-2.5 rounded-2xl font-mono font-bold text-lg shadow-sm">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="bg-[#FF2625] hover:bg-[#e0201f] text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-md shadow-red-500/20 flex items-center gap-1.5 cursor-pointer transition-all"
            >
      ...
      <div className="bg-gray-900 text-white rounded-2xl p-4 flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {[30, 60, 90, 120, 180].map((sec) => (
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-gray-800 hover:bg-[#FF2625] text-xs font-bold font-mono transition-colors cursor-pointer"
            >
              {sec}s
            </button>
          ))}
      ...
      <div className="space-y-6">
        {exercises.map((exercise, exIndex) => {
          const bestSet = exercise.sets.reduce(
            (max, s) => (s.completed && s.weight > max.weight ? s : max),
            { weight: 0, reps: 0 }
          );
                      ...
                      return (
                        <tr
                          ...
                          className={`transition-colors ${
                            set.completed ? 'bg-emerald-50/50' : 'hover:bg-gray-50/60'
                          }`}
                        >
      ...
      {searchModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            {/* Results List */}
            <div className="flex-grow overflow-y-auto p-6 divide-y divide-gray-100">
              {isSearchLoading ? (
